    ...
    : ControllerBase
{
    ...
    [HttpGet("gst-summary")]
    public async Task<IActionResult> GetGstSummary(DateTime? fromDate, DateTime? toDate, string? billingType)
    {
        var hospitalId = GetHospitalId();
        var query = _context.Billings.AsNoTracking()
            .Where(x => x.HospitalId == hospitalId && x.Status == "Paid");
        ...
        if (!string.IsNullOrWhiteSpace(billingType)) query = query.Where(x => x.BillingType == billingType);

        var rows = await query.ToListAsync();
        var breakdown = rows.GroupBy(x => x.BillingType).Select(g => new
        {
            billingType = g.Key,
            bills = g.Count(),
        ...
        }).OrderBy(x => x.billingType).ToList();

        return Ok(new
        {
            totalBills = rows.Count,
            subTotal = rows.Sum(x => x.SubTotal),
            totalGstCollected = rows.Sum(x => x.GstAmount),
            grandTotal = rows.Sum(x => x.TotalAmount),
            breakdown
        });
    }
