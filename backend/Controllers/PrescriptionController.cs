...
[ApiController]
[RequirePermission("Prescriptions")]
    ...
    : ControllerBase
{
            ...
            CreatePrescriptionDto dto)
    {
        try
        {
            ...
            // =============================================
            // SAVE LAB TEST ORDERS
            // =============================================

            foreach (var test in dto.LabTests ?? new List<CreateLabTestOrderDto>())
            {
                if (string.IsNullOrWhiteSpace(test.TestName)) continue;

                _context.PrescriptionLabTests.Add(new PrescriptionLabTest
                {
                    PrescriptionId = prescription.Id,
                    TestName = test.TestName.Trim(),
                    Instructions = test.Instructions?.Trim(),
                    Priority = string.IsNullOrWhiteSpace(test.Priority) ? "Routine" : test.Priority.Trim(),
                    Status = "Ordered",
