...
public class EmailHelper
{
    ...
    public async Task SendStaffCredentials(string toEmail, string name, string role, string temporaryPassword)
    {
        var email = new MimeMessage();
        email.From.Add(new MailboxAddress("Clinic Management System", _config["EmailSettings:Email"]));
        email.To.Add(MailboxAddress.Parse(toEmail));
        email.Subject = $"{role} account created - Clinic Management System";
        email.Body = new TextPart("html")
        {
            Text = $@"<h2>Welcome to Clinic Management System</h2>
<p>Dear {name},</p>
<p>Your <b>{role}</b> account has been created successfully.</p>
<p><b>Email:</b> {toEmail}</p>
<p><b>Temporary Password:</b> {temporaryPassword}</p>
<p>Please login using <b>/api/Auth/login</b> and change your password immediately.</p>
<p>Regards,<br/>Clinic Management Team</p>"
        };
